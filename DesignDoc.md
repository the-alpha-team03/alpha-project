...
* D.1.2 If the backend sends the failure or the wrong response with an error message in it, The frontend will display the error message which is received from the backend.
* D.1.3 After the user is registered, Then the user will be able to login with the correct credentials. When the user clicks on the login button after entering the details, Then it sends the details (entered by the user) to the backend. Then backend will verify the user credentials (by looking to the database). If the details are correct, the backend will send a successful response, or else a failed response.
